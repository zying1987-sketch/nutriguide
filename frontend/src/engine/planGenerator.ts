...
export function generateAIPrompt(result: AssessmentResult, plan: GeneratedPlan): string {
  ...
  // 饮食建议 — 精简版
  prompt += `## 饮食改善重点（精简，最多5条，每条≤35字）\n`
  prompt += `不要按周拆分，给出最关键的3-5个行动项即可（吃什么/不吃什么/怎么吃）。\n\n`

  // 生活方式 — 精简版
  prompt += `## 生活方式（最多3条，每条≤25字）\n`
  prompt += `运动/睡眠/压力各给1条最关键的建议。\n\n`

  // 输出要求
  prompt += `## 输出要求\n`
  prompt += `生成一份**简洁**的结构化报告（总字数控制在600-800字以内）：\n`
  prompt += `1. **用户画像**：1句话，≤80字\n`
  prompt += `2. **核心问题**：3-5条简洁诊断（每条≤40字）\n`
  prompt += `3. **营养素补充**（最重要，用表格）：🔴必须/🟡建议/🟢可选，注剂量+时间+注意事项\n`
  prompt += `4. **饮食重点**（精简，最多5条行动项）\n`
  prompt += `5. **生活方式**（最多3条）\n`
  prompt += `6. **复查建议**（1-2行）\n\n`
  prompt += `风格：像一张快速参考卡片，不要长篇大论。用具体食物名称代替"蔬菜水果"等废词。\n`
  prompt += `结尾注明："> ⚠️ 本方案由AI生成，不替代执业医师诊断。补充剂使用前请咨询医生或注册营养师。"\n`
